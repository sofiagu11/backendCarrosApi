...
export class ControladorRentas{
    ...
    async buscarTodas(request,response){
        ...
        try{
            ...
            response.status(400).json({
                "estado" : "false",
                "mensaje" : "Error buscando los Rentas: "+error,
                "datos" : null
            })
        }
    }
    ...
    async modificar(request,response){
        try{
            ...
            response.status(200).json({
                "estado" : "true",
                "mensaje" : "Exito Modificando los Rentas",
                "datos" : " " // ver la respuesta de await
            })
        }catch(error){
            response.status(400).json({
                "estado" : "false",
                "mensaje" : "Error Modificando los Rentas: "+error,
                "datos" : null
            })
        }
    }
    async registrar(request,response){
        ...
        try{
            ...
            response.status(200).json({
                "estado" : "true",
                "mensaje" : "Exito Registrando los Rentas",
                "datos" : null,
            })
        }catch(error){
            response.status(400).json({
                "estado" : "false",
                "mensaje" : "Error Registrando los Rentas: "+error,
                "datos" : null
            })
        }
    }
    async eliminar(request,response){
        ...
        try{
            ...
            response.status(200).json({
                "estado" : "true",
                "mensaje" : "Exito eliminando los Rentas",
                "datos" : null
            })
            ...
            response.status(400).json({
                "estado" : "false",
                "mensaje" : "Error eliminando los Rentas: "+error,
                "datos" : null
            })
        }
    }
